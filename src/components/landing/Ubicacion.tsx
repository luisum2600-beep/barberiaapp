      ...
      <div className="max-w-screen-xl mx-auto">
        ...
        >
          ...
          <div className="p-8 flex flex-col gap-6" style={{ background: 'var(--clr-bg)' }}>
            <div>
              <p className="section-label mb-3" style={{ color: 'var(--clr-amber)' }}>Dirección</p>
              {barberia.direccion ? (
                <p className="text-sm leading-relaxed" style={{ color: 'var(--clr-gray-300)' }}>
                  {barberia.direccion}
                </p>
              ) : (
                <p className="text-sm" style={{ color: 'var(--clr-gray-700)' }}>No disponible</p>
              )}
            </div>

            {barberia.telefono && (
              <div>
                <p className="section-label mb-1" style={{ color: 'var(--clr-amber)' }}>Teléfono</p>
                <a href={`tel:${barberia.telefono}`} className="text-sm" style={{ color: 'var(--clr-gray-300)' }}>
                  {barberia.telefono}
                </a>
              </div>
            )}
            ...
            {barberia.whatsapp && (
              <a
                ...
                className="inline-flex items-center gap-3 px-5 py-3 mt-2 text-xs font-semibold w-fit transition-all btn-neo-primary"
              >
          ...
          <div className="p-8" style={{ background: 'var(--clr-bg)' }}>
            <p className="section-label mb-4" style={{ color: 'var(--clr-amber)' }}>Horarios</p>
            <div className="space-y-2">
              {Object.entries(horario).map(([dia, h]) => (
                ...
                >
                  <span className="text-xs font-medium" style={{ color: 'var(--clr-gray-500)' }}>
                    {DIAS_ES[dia] ?? dia}
                  </span>
                  {h ? (
                    <span className="text-xs" style={{ color: 'var(--clr-cream)' }}>
                      {h.apertura} – {h.cierre}
                    </span>
                  ) : (
          ...
          <div className="p-8 flex flex-col" style={{ background: 'var(--clr-bg)' }}>
            {barberia.google_maps_url ? (
              ...
              <div
                className="flex-1 flex flex-col items-center justify-center gap-5 min-h-48"
                style={{ border: '1px dashed var(--clr-border)' }}
              >
                <p className="section-label text-center" style={{ color: 'var(--clr-gray-500)' }}>
                  Reserva tu cita ahora
                </p>
                <Link
                  href={`/${barberia.slug}/agendar`}
                  className="font-display text-2xl px-6 py-3 btn-neo-primary"
                >
                  AGENDAR →
                </Link>
              </div>
        ...
        >
          <div className="flex items-center gap-3">
            <LogoFlowPE size={34} />
            <span className="font-display text-xl" style={{ color: 'var(--clr-gray-700)' }}>
              {barberia.nombre}
            </span>
          </div>
          <p className="section-label" style={{ color: 'var(--clr-gray-700)' }}>
